...
The yield itself barely depends on ancestry once sample size is held equal (Fig.~\ref{fig:yield}). A second independent draw of 87 Europeans produced 2,219 usable models, so the two European draws span 2,219 to 2,286 and the Yoruba count of 2,230 falls inside that range. The difference between two random draws of one population is therefore larger than the difference between populations. The same holds at every stage of the funnel: 6,112 to 6,418 genes clear cross-validation in each set, and roughly two thirds of those are then lost to an empty final fit, in every set alike.
...
Size alone, however, is misleading, and separating the two required a within-ancestry control. Two independent random draws of 87 European individuals differ in nothing but sampling, so by construction their frequency and LD components are exactly zero and their weights component measures training noise alone. Across 699 genes this floor gave a median $|\phi_w|$ of 0.545, against 0.752 between ancestries. Restricted to the 356 genes present in both comparisons, counting every gene for which the decomposition was attempted rather than only those with a complete three-way split, the values were 0.446 and 0.633, a ratio of 0.704 (Mann--Whitney $p = 5.8 \times 10^{-6}$). About 70\% of the apparent weights difference between ancestries is therefore reproduced by resampling a single population. The comparison that matters most is blunter still: the noise floor of the weights term, 0.545, exceeds the median frequency and LD components added together, 0.386. The largest component of the decomposition is dominated by training noise, while the two smaller ones are not.
...
The decomposition was repeated using a second independent draw of 87 European individuals against the same Yoruba sample, so that anything changing between the two runs is a property of the draw rather than of ancestry (Fig.~\ref{fig:replicates}). At the level of the population the answer is stable: the median share of the total was 0.639 then 0.654 for the weights, 0.211 then 0.199 for allele frequency, and 0.103 then 0.101 for LD.

Gene by gene it is not. Among the 353 genes with a complete decomposition in both draws, the largest of the three components changed for one gene in four (agreement 75.4\%). Of those genes, 227 were weights-dominated in both draws, 34 frequency-dominated in both and 5 LD-dominated in both, while 87 switched. The correlation between a gene's weights share in one draw and in the other was only Spearman 0.52, with a median absolute change of 0.13.

This bounds what the decomposition supports. The population-level statements above are reproducible across draws of the same population. A statement of the form ``for this gene the disagreement is driven by allele frequency'' is not: at this sample size it has roughly a one in four chance of changing under a different random draw of the same population.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig20_replicates}
\caption{How much the answer depends on the draw. The decomposition is run twice against the same Yoruba sample, using two independent random draws of 87 European individuals. (a) Each gene's weights share in one draw against the other. (b) Which component is largest for a gene, one draw against the other. (c) The median share of each component, one dot per draw.}
...
The decomposition is a reliable population-level summary but not a per-gene one. Repeating it with a second independent draw of 87 European individuals changed the largest of the three components for one gene in four, and a gene's weights share moved by a median of 0.13 between draws. Individual genes should therefore not be labelled frequency-driven or weight-driven at this sample size, and none of the conclusions here rest on a single gene.
